...
class FlavaModel:
	def __init__(self, root_dir, device):
		root_dir = os.path.join(root_dir, "flava")
	...
	def get_text_embeddings(self, texts, text_batch_size=64, normalize=False):
		num_text = len(texts)
		text_embeds = []
		for i in tqdm(range(0, num_text, text_batch_size), desc="Computing text embeddings"):
			text = texts[i: min(num_text, i + text_batch_size)]
			text_input = self.tokenizer(text=text, return_tensors="pt", padding="max_length", max_length=77).to(
				self.device)
			...
			for i_option in batch["image_options"]:
				...
				for c_option in batch["caption_options"]:
					inputs = self.processor(text=list(c_option), images=list(i_option), return_tensors="pt",
											...
											return_image_mask=True).to(self.device)
