...
def get_model(model_name, device, root_dir=CACHE_DIR, dataset_name="f30k"):
	"""
	...
	if model_name == "clip":
		...
		blip_model = BLIPModel(weight_dir=root_dir, device=device)
		...
		xvlm_model = XVLMModel(weight_dir=root_dir, device=device)
		image_preprocess = transforms.Compose([
		...
		llava_model = LLaVAModel("/root/wf/SNARE_VLP_Knowledge_Probing/weight/LLaVA-7B-v0", device=device)
		...
		blip2_model = BLIP2Model("/root/wf/SNARE_VLP_Knowledge_Probing/weight", device=device)
		image_preprocess = blip2_model.vis_processors["eval"]
	...
	# 	vilt_model = ViLTModel(weight_dir=root_dir, device=device)
	# 	image_preprocess = \
