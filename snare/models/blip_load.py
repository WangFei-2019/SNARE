...
from .blip_.model_retrieval import blip_retrieval_load
...
class BLIPModel:
	def __init__(self, weight_dir="weight", config_path="snare/models/blip_/configs", device="cpu"):
		self.weight_dir = weight_dir
		self.model_path = os.path.join(weight_dir, "blip", "model_base_14M.pth")
		self.config_path = os.path.join(config_path, "pretrain.yaml")
		self.bert_config_path = os.path.join(config_path, "med_config.json")

		self.download(os.path.exists(self.config_path), os.path.exists(self.model_path), os.path.exists(
		...
		model = blip_retrieval_load(pretrained=self.model_path,
	...
	def download(self, config_path_exist, model_path_exist, bert_config_path_exist):
		if not config_path_exist:
			os.makedirs(os.path.join(self.weight_dir, "configs"), exist_ok=True)
			print(f"Downloading BLIP config to {self.weight_dir}/blip/configs...")
			config_url = download_urls["config_url"]
			self.config_path = os.path.join(self.weight_dir, "blip", "configs", "pretrain.yaml")
			subprocess.call(["wget", "-c", config_url, "-O", self.config_path])
		if not model_path_exist:
			print(f"Downloading BLIP Model to {self.weight_dir}...")
			model_url = download_urls["model_url"]
			subprocess.call(["wget", "-c", model_url, "-O", self.model_path])
		if not bert_config_path_exist:
			os.makedirs(os.path.join(self.weight_dir, "blip", "configs"), exist_ok=True)
			print(f"Downloading bert config to {self.weight_dir}/blip/configs...")
			bert_config_url = download_urls["bert_config_url"]
			self.bert_config_path = os.path.join(self.weight_dir, "blip", "configs", "med_config.json")
