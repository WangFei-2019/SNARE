...
class ViLTModel:
	def __init__(self, root_dir="weight", config_path="snare/models/vilt/configs", device="cpu"):
		self.root_dir = root_dir
		self.model_path = os.path.join(root_dir, "vilt", "vilt_200k_mlm_itm.ckpt")
	...
	def download(self, config_path_exist, model_path_exist):
		if not config_path_exist:
			...
			print(f"Downloading ViLT config to {self.root_dir}/vilt/configs...")
			config_url = download_urls["config_url"]
			self.config_path = os.path.join(self.root_dir, "blip", "configs", "vilt_200k_mlm_itm.yaml")
			subprocess.call(["wget", "-c", config_url, "-O", self.config_path])
		if not model_path_exist:
			print(f"Downloading ViLT Model to {self.root_dir}...")
