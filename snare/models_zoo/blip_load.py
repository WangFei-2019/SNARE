...
from .blip.model_retrieval import blip_retrieval_load
from snare.models_zoo.utils import MetricLogger
...
class BLIPModel:
	def __init__(self, root_dir="models", config_path="snare/models/blip_/configs", device="cpu"):
		self.root_dir = root_dir
		self.model_path = os.path.join(root_dir, "blip", "model_base_14M.pth")
	...
	def download(self, config_path_exist, model_path_exist, bert_config_path_exist):
		if not config_path_exist:
			...
			print(f"Downloading BLIP config to {self.root_dir}/blip/configs...")
			config_url = download_urls["config_url"]
			self.config_path = os.path.join(self.root_dir, "blip", "configs", "pretrain.yaml")
		...
		if not bert_config_path_exist:
			os.makedirs(os.path.join(self.root_dir, "blip", "configs"), exist_ok=True)
			print(f"Downloading bert config to {self.root_dir}/blip/configs...")
			bert_config_url = download_urls["bert_config_url"]
			self.bert_config_path = os.path.join(self.root_dir, "blip", "configs", "med_config.json")
