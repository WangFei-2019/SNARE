...
class XVLMModel:
	def __init__(self, root_dir="root_dir", config_path="snare/models/xvlm/configs", device="cpu"):
		self.root_dir = root_dir
		self.config_path = os.path.join(config_path, "Pretrain_XVLM_base_4m.yaml")
		self.model_path = os.path.join(root_dir, "xvlm", "4m_base_model_state_step_199999.th")
	...
	def download(self, config_path_exist, model_path_exist):
		if not config_path_exist:
			...
			print(f"Downloading X-VLM config to {self.root_dir}/xvlm/configs...")
			config_url = download_urls["config_url"]
			self.config_path = os.path.join(self.root_dir, "xvlm", "configs", "Pretrain_XVLM_base_4m.yaml")
		...
		if not model_path_exist:
			print(f"Downloading X-VLM Model to {self.root_dir}/xvlm...")
			os.makedirs(os.path.join(self.root_dir, 'xvlm'), exist_ok=True)
		...
		if not os.path.exists(self.config["vision_config"]):
			self.config["vision_config"] = os.path.join(self.root_dir, "xvlm", "configs", "config_swinB_224.json")
			print(f"Downloading X-VLM vision config to {self.root_dir}/xvlm/configs...")
		...
		if not os.path.exists(self.config["text_config"]):
			self.config["text_config"] = os.path.join(self.root_dir, "xvlm", "configs", "config_swinB_224.json")
			print(f"Downloading bert config to {self.root_dir}/xvlm/configs...")
